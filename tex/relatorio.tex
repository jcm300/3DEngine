...
\end{verbatim}
Tal como a estrutura dos modelos, esta lista ligada também é percorrida na fase de renderização. O carater \textit{t} serve para guardar qual o tipo de transformação em causa na altura da renderização enunciada em cima, podendo tomar qualquer um dos seguintes valores:
\begin{itemize}
...
\end{verbatim}
Sendo assim, o carater que identifica esta processo é o 't' e o número de parâmetros a realizar nesta transformação é 3 (\textit{auxT-$>$args =(float*)malloc(sizeof(float)*3);}).
\newline
