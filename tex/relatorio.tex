...
é de 5s e nos encontramos 11s após o início do programa, então já 2 rotações de 360º foram efetuadas e encontramo-nos agora a 80\% da realização da terceira, sendo que, neste caso, \texttt{gt} seria igual a \texttt{0.2}.
\begin{verbatim}
...
Na terceira fase foi realizado os Bezier Patches bem como a translação usando Catmull Rom e a rotação de 360º durante algum tempo. Contudo algumas coisas a melhorar seria, verificação de sintaxe dos ficheiros .patch algo que não é realizado sendo que o generator assume que o ficheiro tem a sintaxe correta, logo quando não a tem o resultado é inesperado. Para além disso poderia ser adicionado as rotações também aos planetas e luas algo que só é feito ao Sol.
