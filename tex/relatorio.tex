...
\end{itemize}
Já o \textit{array} de \textit{float's args} guarda os parâmetros a utilizar na transformação, podendo o seu tamanho ser 3 (\textit{translate} e \textit{scale}), 4 (\textit{rotate}), \textit{NULL} (no caso do \textit{push} e \textit{pop}) ou 1(grupo models guardando o número de modelos).
...
\section{Conclusão}
Apesar do bom trabalho realizado, cumprindo o estipulado no enunciado e sendo que já é usado VBO's de modo a desenhar os pontos, fica por realizar a verificação dos parâmetros passados como input ao generator, visto que, por exemplo o mesmo aceita valores negativos para as slices e stacks do cone. Nesta segunda fase realizou-se a tentativa de melhorar o relatorio anteriormente enviado, tentando assim apresentar fórmulas matemáticas para cada polígono (plane, box, sphere, cone). Foram construidos novos ficheiros de teste visto que os anteriormente apresentados não possuiam a sintaxe final correta. Um destes novos ficheiros de teste é o \textit{solarSytem} no qual pretende representar o sistema solar. Alguns pontos a melhorar nesta 2ºfase é verificar se a sintaxe do ficheiro XML é correta e também verificar se os ficheiros .3d chamados no ficheiro XML existem. Sendo assim, nesta 2º fase se a sintaxe for correta e os ficheiros .3d existirem o resultado será o esperado, caso contrário poderá obter-se resultados inesperados. 
